...
async function targetLink(type: string, id: string) {
  ...
  if (type === "Answer") {
    const answer = await db.answer.findUnique({ where: { id }, select: { questionId: true } });
    return answer ? `/questions/${answer.questionId}#answer-${id}` : null;
  }
  if (type === "Comment") {
    const comment = await db.comment.findUnique({ where: { id }, select: { questionId: true, answer: { select: { questionId: true } } } });
    const questionId = comment?.questionId ?? comment?.answer?.questionId;
    return questionId ? `/questions/${questionId}#comment-${id}` : null;
  }
  if (type === "User") {
    const user = await db.user.findUnique({ where: { id }, select: { username: true } });
    return user ? `/users/${encodeURIComponent(user.username)}` : null;
  }
  return null;
}
...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ status?: "PENDING" | "RESOLVED" | "IGNORED" }> }) {
  ...
  const links = await Promise.all(rows.map((report) => targetLink(report.targetType, report.targetId)));
  return <>
    ...
    <div className="mt-6 space-y-4">{rows.map((report, index) => {
